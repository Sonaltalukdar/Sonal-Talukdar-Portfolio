export const profile = {
  name: 'Sonal Talukdar',
  title: 'Aspiring AI Full Stack Engineer',
  location: 'Hooghly, West Bengal',
  email: 'sonaltalukdar29@gmail.com',
  phone: '9330375596',
  linkedin: 'https://www.linkedin.com/in/sonal-talukdar-b0b998391/',
  github: 'https://github.com/Sonaltalukdar',
  site: 'https://sonaltalukdar.netlify.app',
  about:
    'Enthusiastic B.Tech CSE student building full-stack, AI-integrated web apps with the MERN stack. Comfortable across React, Node.js, Express, MongoDB and Python, with a growing focus on shipping AI-powered features and secure, scalable products.',
}

export const techStack = {
  frontend: ['React.js', 'Tailwind CSS', 'HTML5', 'CSS3', 'JavaScript'],
  backend: ['Node.js', 'Express.js', 'Python (FastAPI)'],
  database: ['MongoDB', 'SQL'],
  languages: ['Java', 'Python', 'C'],
  tools: ['Git', 'GitHub', 'Groq SDK', 'Vite'],
}

export const projects = [
  {
    title: 'AtmosAI',
    subtitle: 'AI-Powered Weather Forecasting Platform',
    tech: ['React.js', 'Tailwind CSS', 'Vite', 'Node.js', 'Express.js', 'MongoDB Atlas', 'Groq (LLaMA 3.3 70B)'],
    points: [
      'Full-stack MERN weather platform with real-time forecasts, AQI and interactive maps',
      "Natural-language weather assistant powered by Groq's LLaMA 3.3 70B",
      'Secure JWT auth with user profiles and favorite locations',
      'Glassmorphism UI inspired by Apple Weather and Nothing OS',
    ],
    github: 'https://github.com/Sonaltalukdar/AtmosAI',
    live: 'https://atmosai-frontend.onrender.com/home',
  },
]

export const education = [
  {
    degree: 'B.Tech, Computer Science & Engineering',
    school: 'Hooghly Engineering and Technology College · MAKAUT, West Bengal',
    period: '2024 — Expected 2027',
  },
  {
    degree: 'Diploma in Computer Science & Technology',
    school: "Women's Polytechnic Chandernagore · WBSCTVESD",
    period: '2021 — 2024',
    note: 'CGPA: 8.3',
  },
  {
    degree: 'Secondary Education',
    school: 'G.D. Birla Centre For Education · ICSE',
    period: '2021',
    note: '79.5%',
  },
]

export const certificates = [
  { title: 'Skill Development & Internship — E-commerce Website Development', period: '1 Mar – 8 Apr 2024' },
  { title: 'Industrial Training — Web Development using Python & Django', period: '1 Mar – 8 Apr 2024' },
  { title: 'AI for Future Workforce Program', period: 'Aug 2023' },
  { title: 'EduSkills Google AI-ML', period: '30 Jan – 1 Feb 2025' },
  { title: 'Google Cloud Facilitator Program', period: '1 Apr – 2 Jun 2025' },
  { title: 'Generative AI, Deep Learning & LLMs — Virtual Internship', period: 'Apr – Jun 2026' },
]